...
检索阶段（向量 + BM25）有个根本矛盾：**要快要全，就难以精准**。
...
flowchart LR
    subgraph Fast["追求速度"]
        A["HNSW 近似最近邻"] --> |"牺牲少许精度"| B["毫秒级响应"]
    end
    subgraph Accurate["追求精度"]
        C["暴力枚举所有文档"] --> |"O(n) 复杂度"| D["秒级甚至分钟级"]
    end
```

向量数据库用 HNSW 索引做近似搜索，它是"找大概相近的"，不是"找最相近的"。百万级文档下，HNSW 可能漏掉真正最相关的那个，或者把次相关的排在前面。

**2. Embedding 是"粗粒度"匹配**

Embedding 把整句话压成一个 1024 维的向量。这个过程中丢失了细节：
- "App Router 的路由方式" 和 "Pages Router 的路由方式" 在向量空间里很近
- 但你的问题是问 App Router 的，向量检索可能把 Pages Router 的文档也排进 Top 5
- Embedding 只知道"都是讲路由的"，分辨不出"具体讲的是哪个路由"

**3. BM25 只管词频，不管语义**

BM25 看到 "hydration failed" 给高分，但它不知道：
- 文档A讲的是 "hydration failed in React 18"（相关）
- 文档B讲的是 "hydration failed in older Next.js versions"（不太相关，因为用户问的是新版）

BM25 对两者打一样的分，因为它只看词出现没出现，不看上下文含义。

**4. Top 5 的"信噪比"不够高**

假设检索召回 20 个候选，真正高度相关的只有 4~5 个。问题是：这 4~5 个不一定全在 Top 5 里，可能第 6、第 8 名其实更相关。

| 排名 | 文档 | 实际相关度 | 检索阶段排名 |
|-----|------|----------|------------|
| 1 | App Router 概述 | 中 | 向量第 1 |
| 2 | Routing 基础 | 低 | BM25 第 1 |
| 3 | App Router vs Pages | **高** | 向量第 5 |
| 4 | Dynamic Routes | 低 | 向量第 3 |
| 5 | Middleware | 低 | BM25 第 2 |

上表里，真正最相关的"App Router vs Pages"排到了第 3，但如果我们只取 Top 5，它已经差点被挤出去了。如果取 Top 20 再精筛，它就不会被漏掉。
...
```mermaid
flowchart LR
    subgraph 招聘流程["招聘流程 = RAG 两阶段检索"]
        direction TB
        A["HR 筛简历<br/>（检索阶段）"] --> |"选出 20 份<br/>'大概符合'的"| B["技术面试<br/>（Rerank 阶段）"]
        B --> |"精筛出 5 份<br/>'真正匹配'的"| C["CEO 终面<br/>（LLM 生成）"]
    end
```

- **HR 筛简历** = 检索：快速、规则化（看学历、年限），不深入细节
- **技术面试** = Rerank：深入考察能力匹配度，花时间但精准
- **CEO 终面** = LLM：基于技术面试的结果做最终决策

如果没有技术面试（Rerank），CEO（LLM）只能基于 HR 筛出来的简历做判断——可能把简历写得好的水货招进来，漏掉真正有能力但简历一般的。

**Rerank 的本质**就是给检索结果做一次"技术面试"：用更贵的计算、更精准的模型，把真正相关的文档挑出来排到前面。
